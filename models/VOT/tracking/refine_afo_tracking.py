...
def get_track_state(
    ...
    if frame_id - state["last_seen"] > timeout:
...
def refine_tracking_results(
    ...
    for txt_file in seq_txt_files:
        ...
        try:
            ...
            logger.warning(f"Could not read {txt_file}")
            continue

        preds = np.atleast_2d(preds)

        if preds.size == 0 or preds.shape[1] < 2:
            logger.warning(f"Invalid prediction file: {txt_file}")
            continue
        ...
        for frame_id in process_bar:
            ...
            for det in current_dets:
                ...
                track_state = get_track_state(
                    track_states=track_states,
                    track_id=track_id,
                    frame_id=frame_id,
                    action_set=action_set,
                    history_length=10,
                    timeout=TRACK_HISTORY_TIMEOUT,
                )

                # # TELEPORTATION CHECK
                # if "last_box" in track_state:
                ...
                #     if overlap < 0.05:
                #         logger.debug(
                #             f"Track {track_id} teleported "
                #             f"(IoU={overlap:.3f}), resetting history."
                #         )

                #         track_state["history"] = ActionHistory(
                #             action_set,
                #             history_length=10,
                #         )
                ...
                track_state["history"] = result["history"]

                track_state["last_box"] = result["final_box"]
                track_state["last_seen"] = frame_id
...
if __name__ == "__main__":
    ...
    KALMAN_RESULTS = f"out/track_res_mamba/afo/{SPLIT}/afo_byte/"
    OUTPUT_RESULTS = f"out/track_res_mamba/afo/{SPLIT}/afo_byte_refined/"
